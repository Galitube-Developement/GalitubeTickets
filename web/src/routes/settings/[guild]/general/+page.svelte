<script>
	...
	settings.workingHours = settings.workingHours.map((v, i) =>
		i === 0 ? v : v?.length === 2 ? v : ['00:00', '00:00']
	);
	...
	const submit = async () => {
		try {
			...
			json.workingHours = settings.workingHours;
...
<div class="m-2 mx-auto max-w-lg p-4 text-lg">
	{#if saved}<p role="status" class="my-3 text-green-600">Settings saved.</p>{/if}
	{#if error}
		<ErrorBox {error} />
	{/if}
			...
			<div>
				<label class="font-medium">
					...
					<input
						...
						bind:value={settings.primaryColour}
					/>
				</label>
			</div>
			<div>
			...
			<div>
				<label class="font-medium">
					...
					<input
						type="text"
						class="input form-input"
						bind:value={settings.successColour}
					/>
				</label>
			</div>
			<div>
						...
						<div in:fade out:fade>
							<div class="mx-4">
								...
								<p class="my-3 text-sm">
									Equal start and end times mean closed. An end before the start
									continues into the next day. 00:00–23:59 means open all day.
								</p>
