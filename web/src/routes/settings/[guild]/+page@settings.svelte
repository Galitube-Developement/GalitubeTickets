...
{#snippet botPublic()}
	...
	<a
		...
		target="_blank">https://github.com/Galitube-Developement/GalitubeTickets/issues</a
	>.
{/snippet}
		...
		{#if guild.stats.categories.length === 0}
			<div class="m-4">
				<a href={guild.id + '/categories/new'}>
					<div
						class="link rounded-xl border-2 border-blurple bg-blurple/20 p-2 font-medium"
					>
						<div class="flex items-center gap-2">
		...
		<div class="rounded-xl bg-white p-4 shadow-sm dark:bg-slate-700">
			...
			<div class="m-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
				...
				<div>
					...
					<p class="text-gray-500 dark:text-slate-400">
						{guild.stats.categories.sort((a, b) => b.tickets - a.tickets)[0]?.name ??
							'None'}
					</p>
				</div>
			</div>
