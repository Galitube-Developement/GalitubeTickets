...
module.exports = async client => {
	...
	fastify.decorate('authenticate', async (req, res) => {
		try {
			...
			if (!Number.isFinite(data.expiresAt) || data.expiresAt < Date.now()) throw 'expired';
			if (process.env.INVALIDATE_TOKENS && (!data.createdAt || data.createdAt < new Date(process.env.INVALIDATE_TOKENS).getTime())) throw 'expired';
		} catch (error) {
	...
	const { handler } = await import('../web/build/handler.js');
