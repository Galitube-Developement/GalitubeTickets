...
module.exports = class TicketManager {
	...
	async postQuestions({
		...
		try {
			const {
				working, timestamp, today,
			} = getWorkingHours(category.guild.workingHours);
			if (!working) {
				const key = timestamp ? (today ? 'today' : 'next') : 'closed';
				await channel.send({
					embeds: [new ExtendedEmbedBuilder()
						.setColor(category.guild.primaryColour)
						.setTitle(getMessage(`ticket.working_hours.${key}.title`))
						.setDescription(getMessage(`ticket.working_hours.${key}.description`, { timestamp }))],
				});
			}
