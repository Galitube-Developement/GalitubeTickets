const { safeRedirect } = require('../../lib/auth');
...
module.exports.get = () => ({
	handler: async function (req, res) {
		...
		if (!data.access_token || !Number.isFinite(data.expires_in) || data.expires_in <= 0) return res.code(502).send({ message: 'Discord login failed. Please try again.' });
		...
		if (!user.id) return res.code(502).send({ message: 'Could not load the Discord user.' });
